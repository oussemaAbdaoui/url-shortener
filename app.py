...
def shorten_url():
    ...
    original_preview = (
        original_url[:50] + "..." if len(original_url) > 50 else original_url
    )
    logger.info("url_created", short_code=short_code, original_url=original_preview)
...
def redirect_to_url(short_code):
    ...
    logger.info(
        "redirect_success", short_code=short_code, click_count=stats_store[short_code]
    )
